...
#' @export
forecast_testset = function(intensity_data, first_day, n_weeks = 2, n_sample = 500,
                            clusters = NULL, models = NULL, naive = FALSE,
                            weeks_of_data = 8, database_user, database_password) {
  ...
  # Function to forecast one week of the testset
  forecast_testset_oneweek = function(first_day) {

    # Sample 500 (location, time)-combinations from the intensity data
  ...
  # Run forecast_testset_oneweek function for all given weeks
  f = function(x) forecast_testset_oneweek(first_day = x)
  all_forecasts = lapply(week_firstdays, f)
